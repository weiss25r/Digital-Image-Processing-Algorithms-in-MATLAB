function [out, map16] = ham6(img)
    img4096 = round(img/17);                                             %Trasforma l'immagina RGB in 4096 colori

    [ind16,map16] = rgb2ind(img4096*17,16, 'nodither');                                    %'nodither'
        ...
        for j = y:w
            ...
            switch(err)
                case d
                    ...
                    lastpix = [map16(x+1,1) map16(x+1,2) map16(x+1,3)];
                case rd
                    out(i,j) = 16 + img4096(i,j,1);
                    lastpix = [img4096(i,j,1), lastpix(2), lastpix(3)];
                case gd
                    out(i,j) = 32 + img4096(i,j,2);
                    lastpix = [lastpix(1), img4096(i,j,2), lastpix(3)];
                case bd
                    out(i,j) = 48 + img4096(i,j,3);
                    lastpix = [lastpix(1), lastpix(2), img4096(i,j,3)];
            end
